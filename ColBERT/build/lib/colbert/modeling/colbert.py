...
class ColBERT(BaseColBERT):
    ...
    def doc_dump(self, input_ids, attention_mask, keep_dims="return_mask", to_cpu=False): # return the compressed and full embeddings so they can be dumped elsewhere
        assert keep_dims == "return_mask"

        input_ids, attention_mask = input_ids.to(self.device), attention_mask.to(self.device)
        D_0 = self.bert(input_ids, attention_mask=attention_mask)[0]
        D = self.linear(D_0)
        ...
        if to_cpu:
            return D.cpu(), D_0.cpu(), mask.bool().cpu()
        return D,D_0,mask.squeeze().bool()
    ...
    def _no_augmentation(self,embs):
        embs[:,:,-1] = 0
        return embs
    def _RH_augmentation(self,embs):
        embs[:,:,-1] = -1
        if self.RH is None:
