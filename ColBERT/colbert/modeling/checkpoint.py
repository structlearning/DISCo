...
class Checkpoint(ColBERT):
    ...
    def query_modified(self, *args, to_cpu=False, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                Q_dump, Q,M = super().query_modified(*args, **kw_args)
                return (Q_dump.cpu(), Q.cpu(),M.cpu()) if to_cpu else (Q_dump, Q,M)
    ...
    def doc_modified(self, *args, to_cpu=False, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                D = super().doc_modified(*args, **kw_args)
    ...
    def queryFromText_modified(
        ...
        if bsize:
            ...
            batches = [
                self.query_modified(input_ids, attention_mask, to_cpu=to_cpu)
                for input_ids, attention_mask in batches
            ]
            embs_dump, embs, masks = zip(*batches)
            return torch.cat(embs_dump), torch.cat(embs),torch.cat(masks)

        input_ids, attention_mask = self.query_tokenizer.tensorize(
            queries, context=context, full_length_search=full_length_search
        )
        return self.query_modified(input_ids, attention_mask, to_cpu=to_cpu)
    ...
    def docFromText_modified(
        ...
        if bsize:
            ...
            keep_dims_ = "return_mask" if keep_dims == "flatten" else keep_dims
            batches = [
                self.doc_modified(input_ids, attention_mask, keep_dims=keep_dims_, to_cpu=to_cpu)
                for input_ids, attention_mask in tqdm(
                    text_batches, disable=not showprogress
                )
            ]
            ...
            if keep_dims is True:
                ...
                # NOTE: norm becomes 'inf' if not cast to float32. Done only for assertion. Also, assert fails for atol=1e-4. 
        ...
        input_ids, attention_mask = self.doc_tokenizer.tensorize(docs)
        return self.doc_modified(input_ids, attention_mask, keep_dims=keep_dims, to_cpu=to_cpu)
    ...
    def docFromBaseEmb(
        ...
        assert clustering_mode in ["hierarchical"]
        # keep dims is necessarily "flatten"
        # bsize shouldnt matter
        
        ## this function is a mixture of docFromText and doc (both checkpoint and colbert)
        with torch.no_grad():
            with self.amp_manager.context():
                if self.colbert_config.augment:
                    D = self._RH_augmentation_corpus(embs)
                else:
    ...
    def docFromText_emb(
        ...
        assert keep_dims is True, "keep_dims must be True for docFromText_emb"
        ...
        if bsize:
            ...
            batches = [
                self.doc_dump(input_ids, attention_mask, keep_dims="return_mask", to_cpu=to_cpu)
                for input_ids, attention_mask in tqdm(
                    text_batches, disable=not showprogress
                )
            ]
            compressed,full,masks = zip(*batches)
            D_full = _stack_3D_tensors(full)
            D_compressed = _stack_3D_tensors(compressed)
            masks = _stack_2D_tensors(masks)
            
            # print("shapes:",D_compressed.shape, D_full.shape,masks.shape)
            # print("types:",D_compressed.dtype, D_full.dtype,masks.dtype)
            # print("reverse_indices:",reverse_indices.shape, reverse_indices.dtype)
            return D_compressed[reverse_indices], D_full[reverse_indices], masks[reverse_indices]
                                


        input_ids, attention_mask = self.doc_tokenizer.tensorize(docs)
        return self.doc_dump(input_ids, attention_mask, keep_dims=keep_dims, to_cpu=to_cpu)
...
def _stack_2D_tensors(groups):
    ...
    output = torch.zeros(
        bsize, maxlen, device=groups[0].device, dtype=groups[0].dtype
    )
