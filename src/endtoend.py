    ...
    def run(self):
        ...
        if self.config.baseline.bucket_size==0: # exact
            ...
            if self.config.embedder.mode=="mem":
                ...
                optvec = -2*torch.ones_like(self.embedder.qmasks).to(self.embedder.qembs.device)
                opt_indices = -torch.ones(self.embedder.qembs.size(0),self.k).to(self.embedder.qembs.device)
                opts_scores = -2000*torch.ones(self.embedder.qembs.size(0),self.k).to(self.embedder.qembs.device)

                for i in tqdm(range(self.k), desc="K", total=self.k):
                    temp_optvec = -2*torch.ones_like(self.embedder.qmasks).to(self.embedder.qembs.device)
                    temp_opt_indices = -torch.ones(self.embedder.qembs.size(0)).to(self.embedder.qembs.device)
                    temp_opts_scores = -2000*torch.ones(self.embedder.qembs.size(0)).to(self.embedder.qembs.device)
                    doc_id = 0
                    for cemb, cmask in tqdm(self.embedder.iterate_over_batches(self.device,self.config.embedder.mode),desc="Corpus"):      
                        ...
                        for q_id in tqdm(range(self.embedder.qembs.shape[0]), desc="Processing queries"):
                            ...
                            temp_optvec[q_id] = torch.maximum(temp_optvec[q_id], max_sim_partial.squeeze(0).to(optvec.device))
                    ...
                    optvec.copy_(temp_optvec, non_blocking=True)
                
                opt_inds = opt_inds.cpu()
                opt_scores = opt_scores.cpu()

                # Combine into list of tuples - the way code is written earlier
                opts = [(opt_inds[i], opt_scores[i]) for i in range(opt_inds.shape[0])]
