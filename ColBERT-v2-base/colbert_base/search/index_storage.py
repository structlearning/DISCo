...
from colbert_base.utils.utils import flatten, print_message

from colbert_base.indexing.loaders import load_doclens
from colbert_base.indexing.codecs.residual_embeddings_strided import ResidualEmbeddingsStrided

from colbert_base.search.strided_tensor import StridedTensor
from colbert_base.search.candidate_generation import CandidateGeneration

from .index_loader import IndexLoader
from colbert_base.modeling.colbert import colbert_score, colbert_score_packed, colbert_score_modified, colbert_score_packed_modified
...
class IndexScorer(IndexLoader, CandidateGeneration):
    ...
    def rank(self, config, Q, k, pids=None):
    ...
    def score_pids_modified(self, config, Q, pids, k, opt_vec):
        """
            Always supply a flat list or tensor for `pids`.
        ...
        if Q.size(0) == 1:
            return colbert_score_packed_modified(Q, D_packed, D_mask, opt_vec, config)

        D_padded, D_lengths = StridedTensor(D_packed, D_mask).as_padded_tensor()

        return colbert_score_modified(Q, D_padded, D_lengths, opt_vec, config)