...
from colbert_base.infra import ColBERTConfig
from colbert_base.training.rerank_batcher import RerankBatcher

from colbert_base.utils.amp import MixedPrecisionManager
from colbert_base.training.lazy_batcher import LazyBatcher
from colbert_base.parameters import DEVICE

from colbert_base.modeling.colbert import ColBERT
from colbert_base.modeling.reranker.electra import ElectraReranker

from colbert_base.utils.utils import print_message
from colbert_base.training.utils import print_progress, manage_checkpoints
...
def train(config: ColBERTConfig, triples, queries=None, collection=None):
    ...
    colbert = colbert_base.to(DEVICE)
    colbert_base.train()
    ...
    optimizer = AdamW(filter(lambda p: p.requires_grad, colbert_base.parameters()), lr=config.lr, eps=1e-8)
    optimizer.zero_grad()
...
def set_bert_grad(colbert, value):
    try:
        for p in colbert_base.bert.parameters():
            assert p.requires_grad is (not value)
            p.requires_grad = value
    except AttributeError:
        set_bert_grad(colbert_base.module, value)
